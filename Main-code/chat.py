...
def get_response(msg):
    ...
    if tag=='news':
        ...
        return("1: ", results[1][0],results[1][1],'\n')
    
    if tag=='cricket':
        c = Cricbuzz()
        matches = c.matches()
        for match in matches:
            return(match['srs'],' ',match['mnum'],' ',match['status'])
    ...
    if tag=='LeetCode':
        ...
        for i in data['emp_details']:
            if i["question_number"]==num1:
                # global question_name 
                question_name = i["question_title"]
        link = "solve this https://leetcode.com/problems/"+str(question_name.replace(' ','-')).lower()
        f.close()
        return link 
